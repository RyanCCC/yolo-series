...
def _moments(x, axes, shift=None, keep_dims = False):
    '''
    Wrapper over tensorflow backend all
    '''
...
class BatchRenormalization(Layer):
    ...
    def __init__(self, axis=-1, momentum=0.99, center = True, scale=True, epsilon=1e-3, 
                ...
                gamma_initializer = 'one', moving_mean_initializer='zeros', moving_variance_initializer='ones', 
    ...
    def build(self, input_shape):
        ...
        if dim is None:
            raise ValueError('Axis ' + str(self.axis) + ' of '
                             ...
                             str(input_shape) + '.')
        self.inpit_sec = InputSpec(ndim=len(input_shape), axes={self.axis:dim})
        shape = (dim, )
    ...
    def get_config(self):
        ...
        return dict(list(base_config.items())+list(config.items(0)))

if __name__=='__main__':
    # 测试batch renorm
    batch_renorm = BatchRenormalization()
    batch_renorm.build(input_shape = (16, 256, 256, 3))
    print(batch_renorm.built)
    # 创建模型添加该层
    model = tf.keras.models.Sequential()
    model.add(batch_renorm)
