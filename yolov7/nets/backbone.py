    ...
    def __init__(self, transition_channels, block_channels, n, phi, pretrained=False):
        ...
        if pretrained:
            pretrain_weights = {
                ...
                "x" : './yolov7/checkpoints/yolov7_x_backbone_weights.pth',
            }[phi]
            # checkpoint = torch.hub.load_state_dict_from_url(url=url, map_location="cpu", model_dir="./model_data")
            # self.load_state_dict(checkpoint, strict=False)
            self.load_state_dict(torch.load(pretrain_weights, map_location='cpu'))
            print("Load weights from " + pretrain_weights)
